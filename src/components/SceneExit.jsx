...
export default function SceneExit({
  ...
  const handleMouseDown = useCallback(
    (e) => {
      // 编辑模式未开启时跳过所有拖拽/缩放/旋转
      if (e.button !== 0) { // 非左键（右键拖拽/缩放）
        try { if (localStorage.getItem('shanglu_edit_mode') !== '1') return; } catch { return; }
      }
      if (e.button === 0) {
        // 左键：缩放/拖拽模式下不响应交互
        ...
        // 编辑模式下启动长按旋转计时器
        let editOn = false;
        try { editOn = localStorage.getItem('shanglu_edit_mode') === '1'; } catch { /* */ }
        if (editOn) {
