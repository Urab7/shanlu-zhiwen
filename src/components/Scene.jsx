...
export default function Scene({
  ...
  // 编辑模式：控制拖拽/缩放/旋转/保存布局
  const [editMode, setEditMode] = useState(() => {
    try { return localStorage.getItem('shanglu_edit_mode') === '1'; }
    catch { return false; }
  });
  const toggleEditMode = useCallback(() => {
    setEditMode((prev) => {
      const next = !prev;
      try { localStorage.setItem('shanglu_edit_mode', next ? '1' : '0'); }
      catch { /* ignore */ }
      return next;
    });
  }, []);
  ...
  const handleCloseDialogue = () => {
    ...
    // 卓玛第二章链尾：完成 → 设标记 + 提升信任
    if (dialogue?.dialogueId === 'dlg_zhuoma_r4_close') {
      onSetNpcFlag('zhuoma', 'zhuomaChapter1Completed', true);
      onAdjustTrust('zhuoma', 25);
    }
    // 旦增第二章链尾（理智线）：完成 → 设标记 + 提升信任
    if (dialogue?.dialogueId === 'dlg_danzeng_r3_final') {
      onSetNpcFlag('danzeng', 'danzengChapter2Completed', true);
      onAdjustTrust('danzeng', 25);
    }
    // 白玛第二章链尾：完成 → 设标记 + 提升信任
    if (dialogue?.dialogueId === 'dlg_baima_r4_scripture') {
      onSetNpcFlag('baima', 'baimaChapter2Completed', true);
      onAdjustTrust('baima', 25);
    }
    // 仁青第二章链尾：完成 → 设标记 + 提升信任
    if (dialogue?.dialogueId === 'dlg_renqing_r6_close') {
      onSetNpcFlag('renqing', 'renqingChapter2Completed', true);
      onAdjustTrust('renqing', 25);
    }
    // 第三章会议链尾：完成 → 设标记 + 提升信任
    if (dialogue?.dialogueId === 'dlg_zhuoma_ch3_meeting_close') {
      onSetNpcFlag('zhuoma', 'meetingCompleted', true);
      onAdjustTrust('zhuoma', 10);
    }
    // 第三章：白玛猎人故事链尾
    if (dialogue?.dialogueId === 'dlg_baima_ch3_hunter_close') {
      onSetNpcFlag('baima', 'baimaHunterCompleted', true);
      onAdjustTrust('baima', 10);
    }
    // 第三章：旦增泥手链尾
    if (dialogue?.dialogueId === 'dlg_danzeng_ch3_hands_close') {
      onSetNpcFlag('danzeng', 'danzengHandsCompleted', true);
      onAdjustTrust('danzeng', 10);
    }
    // 第三章：格桑白描链尾
    if (dialogue?.dialogueId === 'dlg_gesang_ch3_sketch_end') {
      onSetNpcFlag('gesang', 'gesangSketchCompleted', true);
      onAdjustTrust('gesang', 10);
    }
    // 第三章：仁青照片分析链尾
    if (dialogue?.dialogueId === 'dlg_renqing_ch3_photo_hint') {
      onSetNpcFlag('renqing', 'renqingPhoto23Completed', true);
      onAdjustTrust('renqing', 10);
    }
    // 第三章：白玛煨桑教学链尾 → 本章收束，引导找钥匙
    if (dialogue?.dialogueId === 'dlg_baima_ch3_weisang_close') {
      onSetNpcFlag('baima', 'weisangCompleted', true);
      onAdjustTrust('baima', 10);
    }
    // 第四章：钥匙对话链尾 → 门已解锁
    if (dialogue?.dialogueId === 'dlg_zhuoma_ch4_key_close'
        || dialogue?.dialogueId === 'dlg_baima_ch4_key_close') {
      onSetNpcFlag('baima', 'hasOpenedTemple', true);
      onSetNpcFlag('zhuoma', 'hasOpenedTemple', true);
      onAdjustTrust(dialogue?.dialogueId === 'dlg_baima_ch4_key_close' ? 'baima' : 'zhuoma', 10);
    }
...
`;
  ...
  return (
    ...
    >
      ...
      {exits.map((exit) => {
        ...
        return (
          <SceneExit
            ...
            onClick={() => {
              // 卧室：需赠送唐卡 / 调试模式
              const bedroomDebugOpen = localStorage.getItem('shanglu_bedroom_debug') === '1';
              const isBedroomUnlocked = (exit.target === 'dangzeng_bedroom'
                && npcTrust?.danzeng?.flags?.hasGivenThangka)
                || (exit.target === 'dangzeng_bedroom' && bedroomDebugOpen);
              // 经堂：需卓玛或白玛知己 + 赠送过唐卡
              if (exit.target === 'temple_interior') {
                const templeDebugOpen = localStorage.getItem('shanglu_temple_debug') === '1';
                if (templeOpened || templeDebugOpen) {
                  if (templeDebugOpen) console.warn('[DEBUG] 经堂调试模式：直接进入');
      ...
      {sceneNpcs.map((npc) => {
        ...
        // 仁青：第三章回归 > 第二章链 > 首次见面 > 话题面板
        if (npc.id === 'renqing') {
          const zhuomaCh1Done = npcTrust?.zhuoma?.flags?.zhuomaChapter1Completed ?? false;
          const renqingCh2Done = trust?.flags?.renqingChapter2Completed ?? false;
          const gesangSketchDone = npcTrust?.gesang?.flags?.gesangSketchCompleted ?? false;
          const renqingPhoto23Done = trust?.flags?.renqingPhoto23Completed ?? false;
          if (gesangSketchDone && !renqingPhoto23Done) {
            dialogueId = 'dlg_renqing_ch3_return_open';
          } else if (zhuomaCh1Done && !renqingCh2Done) {
            dialogueId = 'dlg_renqing_r1_open';
          } else if (!renqingFirstMet) {
        ...
        if (highlight) {
          if (npc.id === 'danzeng') {
            // 第四章结局链：煨桑完成 + 持唐卡 → 深度赠予链
            const weisangDone = npcTrust?.baima?.flags?.weisangCompleted ?? false;
            if (weisangDone && hasReplica) {
              dialogueId = 'dlg_danzeng_ch4_give_open';
            } else {
        ...
        if (!highlight) {
          // 卓玛第二章完成后 → 旦增主线链
          const zhuomaCh1Done = npcTrust?.zhuoma?.flags?.zhuomaChapter1Completed ?? false;
          if (npc.id === 'danzeng' && zhuomaCh1Done && !(trust?.flags?.danzengChapter2Completed ?? false)) {
            dialogueId = 'dlg_danzeng_r1_open';
          }
          // 卓玛第二章完成后 → 白玛主线链
          if (npc.id === 'baima' && zhuomaCh1Done && !(trust?.flags?.baimaChapter2Completed ?? false)) {
            dialogueId = 'dlg_baima_r1_open';
          }
          // 第三章：白玛猎人故事（meeting完成后）
          if (npc.id === 'baima'
            && (trust?.flags?.meetingCompleted ?? false)
            && !(trust?.flags?.baimaHunterCompleted ?? false)) {
            dialogueId = 'dlg_baima_ch3_hunter_open';
          }
          // 第三章：旦增泥手（白玛猎人完成后）
          if (npc.id === 'danzeng'
            && (npcTrust?.baima?.flags?.baimaHunterCompleted ?? false)
            && !(trust?.flags?.danzengHandsCompleted ?? false)) {
            dialogueId = 'dlg_danzeng_ch3_hands_open';
          }
          // 第三章：格桑白描（旦增泥手完成后）
          if (npc.id === 'gesang'
            && (npcTrust?.danzeng?.flags?.danzengHandsCompleted ?? false)
            && !(trust?.flags?.gesangSketchCompleted ?? false)) {
            dialogueId = 'dlg_gesang_ch3_sketch_open';
          }
          // 第三章：白玛煨桑教学（仁青照片完成后）
          if (npc.id === 'baima'
            && (npcTrust?.renqing?.flags?.renqingPhoto23Completed ?? false)
            && !(trust?.flags?.weisangCompleted ?? false)
            && !(dialogueId?.startsWith('dlg_baima_ch3_hunter') ?? false)) {
            dialogueId = 'dlg_baima_ch3_weisang_open';
          }
          // 第四章：白玛经堂钥匙（煨桑完成后）
          if (npc.id === 'baima'
            && (trust?.flags?.weisangCompleted ?? false)
            && !templeOpened) {
            dialogueId = 'dlg_baima_ch4_key_open';
          }
          ...
          // 卓玛：格桑场景四完成后 → 触发第二章主线对话链
          //       三条第二章链都完成后 → 触发第三章会议
          if (npc.id === 'zhuoma') {
            const gesangScene4Done = npcTrust?.gesang?.flags?.gesangScene4Completed ?? false;
            const zhuomaCh1Completed = trust?.flags?.zhuomaChapter1Completed ?? false;
            const meetingDone = trust?.flags?.meetingCompleted ?? false;
            const weisangDone = npcTrust?.baima?.flags?.weisangCompleted ?? false;
            if (gesangScene4Done && !zhuomaCh1Completed) {
              dialogueId = 'dlg_zhuoma_r1_opening';
            } else if (zhuomaCh1Completed
              && (npcTrust?.danzeng?.flags?.danzengChapter2Completed ?? false)
              && (npcTrust?.baima?.flags?.baimaChapter2Completed ?? false)
              && (npcTrust?.renqing?.flags?.renqingChapter2Completed ?? false)
              && !meetingDone) {
              dialogueId = 'dlg_zhuoma_ch3_meeting_open';
            } else if (weisangDone && !templeOpened) {
              dialogueId = 'dlg_zhuoma_ch4_key_open';
            }
          }
  ...
  return (
      ...
      {/* 编辑模式开关 + 保存布局 */}
      <button
        className="scene__edit-toggle"
        onClick={toggleEditMode}
        title={editMode ? '关闭编辑模式（禁用拖拽/缩放/保存）' : '开启编辑模式（允许拖拽/缩放/保存布局）'}
      >
        {editMode ? '🔧 编辑中' : '🔒'}
      </button>
      {editMode && (
