...
export default function DialogueOverlay({ dialogue, itemLabel, isNpc, startInAi, npcIdOverride, onClose, onKeyLineAdded, onAdvanceDialogue }) {
  ...
  // 选项分支推进标记：从选项进入 AI 模式后，退出时需推进到下一段剧情
  const branchAdvanceRef = useRef(null);
  ...
  // 结局演出自动推进：dialogue.autoAdvanceMs 毫秒后自动跳到下一段
  useEffect(() => {
    const ms = dialogue?.autoAdvanceMs;
    if (!ms || !dialogue?.nextDialogueId || !onAdvanceDialogue) return;
    const id = setTimeout(() => {
      onAdvanceDialogue(dialogue.nextDialogueId);
    }, ms);
    return () => clearTimeout(id);
  }, [dialogue?.id, dialogue?.autoAdvanceMs, dialogue?.nextDialogueId, onAdvanceDialogue]);
  ...
  // 玩家选择选项：AI 先产生分支回应 → 玩家可继续自由对话或点「继续剧情」推进
  const handleSelectOption = useCallback((optionText) => {
    ...
    // 进入 AI 模式，预置用户消息 + 空 assistant 占位（避免流式竞态）
    const userMsg = { role: 'user', content: `【玩家选择了：${optionText}】` };

    setMode('ai');
    setAiMessages([userMsg, { role: 'assistant', content: '' }]);
    setAiStreaming(true);
    setAiError(null);

    branchAdvanceRef.current = {
      nextDialogueId: dialogue?.nextDialogueId || null,
      shouldClose: !dialogue?.nextDialogueId,
    };

    // 将 NPC 当前对话作为上下文注入
    const npcContext = lines.map(getLineText).filter(Boolean).join('\n');
    const enhancedSystem = systemPrompt
      + '\n\n--- 当前剧情对话 ---\n' + npcContext
      + '\n\n玩家选择了以下选项，请以角色的身份对此做出回应。保持角色语气和世界观一致。';
    const enhancedMessages = buildMessages(enhancedSystem, [userMsg]);
    ...
    (async () => {
      try {
        await streamChat(
          { messages: enhancedMessages, signal: ac.signal },
          {
            onToken: (token) => {
              setAiMessages((prev) => {
                const next = [...prev];
  ...
  }, [aiAvailable, dialogue?.nextDialogueId, systemPrompt, lines, onAdvanceDialogue, onClose]);
  ...
  const handleEnterAi = useCallback(() => {
    ...
    setMode('ai');
    branchAdvanceRef.current = null; // 手动进入，不是选项分支
  ...
  const handleExitAi = useCallback(() => {
    ...
    // 来自选项分支 → 推进到下一段剧情（微任务延迟确保 mode='fixed' 先提交）
    if (branchAdvanceRef.current) {
      const branch = branchAdvanceRef.current;
      branchAdvanceRef.current = null;
      Promise.resolve().then(() => {
        if (branch.nextDialogueId && onAdvanceDialogue) {
          onAdvanceDialogue(branch.nextDialogueId);
        } else {
          onClose();
        }
      });
  ...
  }, [isLast, onClose, dialogue?.playerOptions, aiMessages, onAdvanceDialogue]);

  // 是否来自选项分支（用于显示「继续剧情」按钮）
  const isBranchMode = mode === 'ai' && branchAdvanceRef.current !== null;
  // 选项分支流式完成、可继续推进
  const canBranchAdvance = isBranchMode && !aiStreaming;
          ...
          <>
            ...
            <div className="dialogue-overlay__ai-input-bar" onClick={(e) => e.stopPropagation()}>
              <input
                ...
                placeholder={isBranchMode ? '继续追问……（或点击右侧按钮推进剧情）' : '问点什么……'}
                value={aiInput}
              ...
              {canBranchAdvance ? (
                <button
                  className="dialogue-overlay__ai-send dialogue-overlay__branch-advance"
                  onClick={handleExitAi}
                >
                  继续剧情 ▶
                </button>
              ) : (
                <button
                  ...
                  disabled={aiStreaming || (!aiInput.trim() && !isBranchMode)}
                >
                  {aiStreaming ? '…' : '发送'}
                </button>
              )}
